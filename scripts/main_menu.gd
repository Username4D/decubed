...
func _ready() -> void:
	...
	ColorPalettes.load_palette(randi_range(0, clamp(len(ColorPalettes.palettes) - 1, 0, floori(SettingsHandler.unlocked_levels / 10))))
...
func _on_settings_button_pressed() -> void:
	...
	await View.transition_midpoint
	await get_tree().process_frame
	await View.change_scene_to_file("res://scenes/settings_menu.tscn", true, true)
